...
Page({
  data: { accounts: [], familyId: "", loading: true, errorMessage: "" },
  ...
  async load() {
    ...
    try {
      const result = await this.call("listAllAccounts", { familyId: this.data.familyId });
      this.setData({ accounts: (result.accounts || []).map((item) => ({ ...item, id: item._id })) });
    } catch (error) {
